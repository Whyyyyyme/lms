...
class SyncScheduledLmsItemsJob
{
    ...
    private function publishScheduledMaterials(StudentAccessService $studentAccess): void
    {
        ...
        Material::query()
            ->with(['kelas.course.studySemester'])
            ->whereNotNull('published_at')
            ->where('published_at', '<=', now())
            ->whereNull('published_notification_sent_at')
            ->chunkById(50, function ($materials) use ($studentAccess): void {
    ...
    private function publishScheduledAssignments(StudentAccessService $studentAccess): void
    {
        ...
        Assignment::query()
            ...
            ->whereNull('published_notification_sent_at')
            ->chunkById(50, function ($assignments) use ($studentAccess): void {
    ...
    private function openScheduledAttendances(StudentAccessService $studentAccess): void
    {
        ...
        Attendance::query()
            ...
            ->where('opened_at', '<=', $now)
            ->where('closed_at', '>', $now)
            ->chunkById(50, function ($attendances) use ($studentAccess): void {
    ...
    private function closeExpiredAttendances(): void
    {
        ...
        Attendance::query()
            ...
            ->whereNotNull('closed_at')
            ->where('closed_at', '<=', now())
            ->chunkById(50, function ($attendances): void {
                foreach ($attendances as $attendance) {
                    $attendance->update([
                        'is_open' => false,
